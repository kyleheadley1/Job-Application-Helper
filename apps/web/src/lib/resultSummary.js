...
const conceptForRisk = (risk) => {
    for (const entry of RISK_CONCEPTS) {
        if (entry.re.test(risk))
            return entry.concept;
    }
    return "other";
};
const tokenSet = (s) => new Set(normalizeText(s)
    .split(" ")
    .filter((t) => t.length > 2));
const similarRisk = (a, b) => {
    const A = tokenSet(a);
    const B = tokenSet(b);
    if (!A.size || !B.size)
        return false;
            ...
            : concept === "domain_business"
                ...
                : concept === "location_logistics"
                    ...
                    : concept === "screen_process"
                        ? 12
                        : 10;
    if (/\b(required|hard|must|cannot|limited|strongly preferred)\b/.test(t))
        score += 8;
    if (/\b(go|language|stack)\b/.test(t))
        score += 8;
    if (/\b(scale|internet scale|infrastructure|production systems)\b/.test(t))
        score += 7;
    return score;
};
export function selectTopFits(job, n = 2) {
    ...
    for (const item of items) {
        ...
        if (classifyReason(candidate) === "negative")
            continue;
...
export function selectDistinctRisks(job, n = 2) {
    const baseItems = [job.mainRisk, ...job.risks].map((s) => s.trim()).filter(Boolean);
    const items = [];
    for (const item of baseItems) {
        const split = splitMixedReason(item);
        if (split.negative)
            items.push(split.negative);
        if (classifyReason(item) !== "positive")
            items.push(item);
    }
    const jdText = normalizeText([job.extracted.title, ...(job.extracted.requirements ?? []), ...(job.extracted.responsibilities ?? []), job.extracted.rawText ?? ""].join(" "));
    ...
    for (const item of items) {
        ...
        if (!strictDegreeSignal && /\b(degree|bachelor)\b/.test(t))
            strength -= 9;
        if (strictDegreeSignal && /\b(degree|bachelor)\b/.test(t))
            strength += 9;
        const matchIdx = deduped.findIndex((d) => d.concept === concept && (similarRisk(d.text, item) || normalizeText(d.text) === normalizeText(item)));
        if (matchIdx >= 0) {
            if (strength > deduped[matchIdx].strength)
                deduped[matchIdx] = { text: item, concept, strength };
            continue;
        }
    ...
    for (const r of deduped) {
        if (usedConcepts.has(r.concept) && deduped.some((x) => !usedConcepts.has(x.concept)))
            continue;
        out.push(r.text);
...
export function decisionSummaryLine(job) {
...
export function displayRoleTitle(rawTitle) {
    const title = rawTitle.trim();
    if (!title)
        return title;
    const looksLikeRoleHeader = /\b(junior|entry[-\s]?level|early[-\s]?career|associate|software|full[-\s]?stack|backend|frontend|product)\b/i.test(title) && /\bengineers\b$/i.test(title);
    if (!looksLikeRoleHeader)
        return title;
