...
export const buildDeterministicGeneratedAssets = (job, profile) => {
    ...
    if (recommendedResume === "SIE") {
        talkingPoints.push("I can walk through how I'd run a technical onboarding or integration slice without hand-wavy architecture slides.");
        talkingPoints.push("I'm comfortable translating fuzzy stakeholder requests into a small set of implementable milestones — then executing.");
        talkingPoints.push("I stay close to delivery risk: dependencies, cutover order, and what to validate early with a customer engineer.");
    }
    else if (recommendedResume === "EARLY_CAREER") {
        talkingPoints.push("I'm early-career with a training-backed foundation; I do best with tight feedback loops and small shipped increments.");
        talkingPoints.push("I've built full-stack project work where the constraint was clarity — scoping, tradeoffs, and readable code.");
        talkingPoints.push("I'm explicit about what I haven't done yet; I'd rather earn trust than bluff depth I don't have.");
    }
    else {
        talkingPoints.push("I've shipped backend-leaning full-stack work where APIs and internal tools were the spine of the product loop.");
        talkingPoints.push("I'm strongest when the job is ambiguous but measurable — narrow the unknowns, ship, then iterate with stakeholders.");
        talkingPoints.push("I keep claims tied to what I've actually built (see projects below) rather than generic 'passion for excellence'.");
    }
