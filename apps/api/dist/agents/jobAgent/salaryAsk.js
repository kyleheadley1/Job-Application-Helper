...
const roundToNearest5k = (n) => Math.round(n / 5_000) * 5_000;
export const computeSalaryAsk = (params) => {
    ...
    if ((rules.credentialHeavyFintechAlgorithm || rules.goDistributedDataInfraCandidateGap) &&
        recommendation === "no") {
        return {};
    }
    if (postedMin && postedMax) {
        const band = postedMax - postedMin;
        let conservativeAdjustment = recommendation === "yes"
            ? 0.55
        ...
        const eliteForTopSalary = score.total >= 82 &&
            score.levelFit >= 10 &&
            score.stackFit >= 20 &&
            !rules.stackMismatch;
        ...
        if (rules.explicitCoreLanguageMismatch === true &&
        ...
        if (rules.backendProductApiRole === true &&
    ...
    const entryLevelAiShape = rules.earlyCareerFriendlyRole ||
        (job.yearsExperience?.min ?? 99) <= 2 ||
        /\b(intern|junior|entry[-\s]?level|associate\s+engineer)\b/i.test(impliedBlob);
    const remoteUsShape = /\b(remote|anywhere(\s+in)?\s+the\s+us|work\s+from\s+home|wfh)\b/i.test(impliedBlob) &&
        /\b(us|u\.s\.|united states|usa)\b/i.test(impliedBlob);
    const explicitlyLowPayingPosted = typeof postedMax === "number" && postedMax > 0 && postedMax < 115_000;
    if (entryLevelAiShape &&
    ...
    if (recommendation === "no")
        base -= 10000;
    if (score.total >= 80 && recommendation === "yes")
        base += 5000;
