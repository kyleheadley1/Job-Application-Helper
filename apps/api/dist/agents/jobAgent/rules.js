...
export const evaluateRules = (job, profile) => {
    const notes = [];
    const penaltyVector = {};
    ...
    const explicitSeniorStaffInJd = /\b(senior|staff|principal|sr\.)\b/i.test(combinedText) ||
        /\b(lead\s+engineer|engineering\s+manager|director\s+of\s+engineering)\b/i.test(combinedText);
    const fivePlusYearsSoftPreferred = /\b(5\+|6\+|7\+|8\+|10\+)\s*years?\b[^.\n]{0,160}\b(preferred|nice to have|a plus|bonus|plus)\b/i.test(combinedText) || /\b(preferred|nice to have)[^.\n]{0,160}\b(5\+|6\+|7\+)\s*years?\b/i.test(combinedText);
    const yearsFivePlusHardRequired = ((job.yearsExperience?.min ?? 0) >= 5 && !fivePlusYearsSoftPreferred) ||
        /\b(5\+|6\+|7\+|8\+|10\+)\s*years?[^.\n]{0,40}\b(required|must)\b/i.test(combinedText) ||
        /\b(at\s+least|minimum)[^.\n]{0,24}(5|6|7|8|10)\s*\+?\s*years?[^.\n]{0,30}\b(required|must)\b/i.test(combinedText);
    const leadingTeamsHard = /\b(people\s+manager|manage\s+engineers|managing\s+engineers|leading\s+a\s+team\s+of\s+engineers)\b/i.test(combinedText);
    const contradictorySeniorSignals = explicitSeniorStaffInJd || yearsFivePlusHardRequired || leadingTeamsHard;
    // Associate/entry roles should not get "overreach" unless the JD also carries explicit senior markers.
    const seniorityOverreach = contradictorySeniorSignals && !earlyCareerFriendlyRole;
    const primaryNonNycMetroInLocationLine = /\b(location|based|office)\s*:\s*[^.\n]{0,100}\b(dallas|austin|seattle|san francisco|sf\b|los angeles|chicago|denver|atlanta|boston|miami|philadelphia|phoenix|detroit|houston|portland)\b/i.test(combinedText);
    const nycOrNjViableInText = textImpliesNycMetroOrCommutableNj(combinedText) && !primaryNonNycMetroInLocationLine;
    ...
    const backendProductApiRole = /\b(backend|api|full[-\s]?stack|product engineer|product engineering|customer problems?|feature development|features|reliable systems?|testing|debugging|production systems?)\b/i.test(combinedText) &&
        !/\b(sre|site reliability|platform engineering|devops|terraform|iac|infrastructure tooling|airgapped|linux internals|container runtime|supply chain security|security hardening)\b/i.test(combinedText);
    const infraCoreRole = /\b(sre|site reliability|platform engineering|devops|infrastructure tooling|kubernetes platform|container runtime|linux internals|terraform|iac|airgapped|supply chain|security hardening)\b/i.test(combinedText) &&
        !/\b(product features?|pm|design|customer problems?|full[-\s]?stack|backend api)\b/i.test(combinedText);
    const stackMismatch = !backendProductApiRole &&
        !includesAny(combinedText, [
            'typescript',
            'javascript',
            'node',
    ...
    const healthcareProductEngineering = /\b(healthcare|health\s+care|behavioral\s+health|clinical|patient|hospital|therapy|ehr|hipaa)\b/i.test(combinedText) &&
        /\b(product\s+engineer|full[-\s]?stack|software\s+engineer|typescript|javascript|react|api|revenue|billing|growth|internal\s+tools|crm)\b/i.test(combinedText);
    const domainMismatch = !healthcareProductEngineering &&
        includesAny(combinedText, [
            'medical billing',
    ...
    const microservicesHeavy = /\b(microservices?|service[-\s]?oriented|distributed systems?|on[-\s]?call|production troubleshooting|incident response)\b/i.test(combinedText);
    ...
    if (isFinance && !fintechGoPrimaryStretch) {
        ...
        penaltyVector.finance = 5;
        notes.push('No prior fintech/payments or co-branded card experience may create a steeper ramp and stricter screening.');
    }
    ...
    if (stackMismatch) {
        ...
        notes.push("Preferred Go/GraphQL/platform stack is not the candidate's strongest lane, but baseline backend expectations appear accessible.");
    }
    else if (backendProductApiRole && !infraCoreRole) {
        const companyLabel = job.company?.trim() || 'This company';
        const matureFintechApi = /\b(plaid|stripe|fintech|payments|banking[-\s]?api|financial infrastructure)\b/i.test(combinedText);
        notes.push(matureFintechApi
        ...
        notes.push('Role appears platform/infra-core; deeper production infra background may be screened more strictly.');
    }
    if (domainMismatch) {
    ...
    if (researchHeavyAiRole) {
        ...
        notes.push('Proven track record of research results is a major recruiter-screen gap.');
    }
    ...
    if (fdeBuilderSoftwarePrimary) {
        notes.push('Forward-deployed / growth title without strong external customer-implementation JD — default builder-first SWE screen story (SIE only as secondary angle).');
    }
    const coreLang = analyzeCoreLanguageRequirement(job, profile);
    const matureStructuredEmployer = isMatureStructuredEmployer(job.company ?? '', combinedText) || harshEmployerContext;
    const explicitCoreLanguageMismatch = matureStructuredEmployer &&
        coreLang.explicitHardRequirement &&
        !coreLang.candidateHasProductionLanguage;
    const knownStrongEmployer = /\b(google|alphabet|meta|facebook|amazon|aws|microsoft|apple|netflix|uber|spotify|salesforce|oracle|ibm|stripe|airbnb|databricks|palantir|openai|anthropic|goldman|jpmorgan|jp\s*morgan|bloomberg)\b/i.test(companyNorm);
    const startupOrWeakEmployerShape = /\b(seed|series\s+[ab]|pre-seed|stealth|early-stage|early stage|startup|founding team|y\s*combinator|yc\s*backed)\b/i.test(combinedText);
    const unknownWeakEmployer = !knownStrongEmployer && (startupOrWeakEmployerShape || companyNorm.length < 36);
    const entryLevelAiSignals = earlyCareerFriendlyRole ||
        (job.yearsExperience?.min ?? 99) <= 2 ||
        /\b(intern|internship|associate\s+engineer|junior|entry[-\s]?level)\b/i.test(combinedText);
    const vagueEarlyStageAiCalibration = !harshEmployerContext &&
        jdHasAppliedAiSystemsOverlap(combinedText) &&
        jdIsStructurallyVague(job) &&
        unknownWeakEmployer &&
        entryLevelAiSignals;
    if (vagueEarlyStageAiCalibration) {
        notes.push('Vague or thin JD for an entry-level AI posting at a non-name employer — scores calibrated down for generic-posting inflation.');
    }
    const hardRuleNotes = [];
    if (locationMismatch) {
...
export const isLikelyTraditionalEmployer = (job) => {
    const blob = normalizeText(`${job.company} ${job.title} ${(job.domainTags ?? []).join(' ')} ${job.rawText ?? ''}`);
