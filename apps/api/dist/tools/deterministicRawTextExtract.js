...
export const parseSalaryFromText = (raw) => {
...
const parseTitleLine = (lines) => {
    ...
    if (/engineer|developer|scientist|architect/i.test(first))
        return first;
    return first.length > 80 ? undefined : first;
};
const parseCompanyFromLine = (lines, hint) => {
    if (hint?.trim())
        return hint.trim();
    if (!lines.length)
        return undefined;
    const first = lines[0].trim();
    const emDash = first.split(/\s*[—–-]\s*/);
    if (emDash.length >= 2 && emDash[0].length > 1 && emDash[0].length < 80)
        return emDash[0].trim();
...
const parseLocationLine = (text) => {
    ...
    if (locMatch) {
        ...
        if (HYBRID.test(low))
            remoteType = "hybrid";
        else if (ONSITE.test(low) || /^onsite\b/i.test(chunk))
            remoteType = "onsite";
        else if (US_REMOTE.test(low) || /^remote\b/i.test(chunk))
            remoteType = "remote";
    ...
    if (remoteFirst) {
        ...
        if (HYBRID.test(low))
            remoteType = "hybrid";
        else if (ONSITE.test(low))
            remoteType = "onsite";
        else if (US_REMOTE.test(low))
            remoteType = "remote";
        return { location: chunk.trim(), remoteType: remoteType === "unknown" ? undefined : remoteType };
    }
    return {};
};
const inferRemoteType = (text) => {
    const n = text.toLowerCase();
    if (HYBRID.test(n))
        return "hybrid";
    if (ONSITE.test(n) && !US_REMOTE.test(n))
        return "onsite";
    if (US_REMOTE.test(n))
        return "remote";
    return "unknown";
...
const sectionLines = (text, header) => {
    ...
    for (let i = idx + 1; i < lines.length; i++) {
        if (/^(requirements|responsibilities|qualifications|what you|benefits|about us)/i.test(lines[i]))
            break;
        if (/^[-•*]\s+/.test(lines[i]) || lines[i].length > 10)
 ...
 */
export const extractFromRawText = (normalizedText, companyHint) => {
    ...
    const keywordReqLines = lines.filter((l) => /\b(bachelor|degree|citizenship|sponsorship|clearance|authorized to work|visa)\b/i.test(l) && l.length < 400);
...
export const listMissingCriticalFields = (e) => {
    const missing = [];
    if (!e.title || e.title === "Unknown Title")
        missing.push("title");
    if (!e.company || e.company === "Unknown Company")
        missing.push("company");
    if (!e.stack?.length && !e.requiredSkills?.length)
        missing.push("stackOrSkills");
    if (e.remoteType === "unknown")
