...
Series B startup. Not a bank.
    ...
    it("classifies research-heavy applied AI researcher roles", () => {
        const rules = evaluateRules(makeJob({
            ...
            stack: ["Python"],
            rawText: "Looking for proven research track record and publications. Focus on self-constructing systems, meta-learning loops, program synthesis, evolutionary computation, model experiments, and frontier research results.",
    ...
    it("flags vagueEarlyStageAiCalibration for thin entry-level AI startup JD", () => {
        const rules = evaluateRules(makeJob({
            ...
            requirements: [],
            rawText: "Remote (US). Seed startup. Generative AI product for SMBs. Great culture. Fast learners welcome.",
            location: "Remote (US)",
        }), userProfile);
    ...
    it("does not set seniority overreach for associate/entry roles with familiarity wording", () => {
        const rules = evaluateRules(makeJob({
            ...
            rawText: "Associate Software Engineer. Preferred qualifications include Go, GraphQL, cloud deployments, Docker/Kubernetes.",
        }), userProfile);
        expect(rules.seniorityOverreach).toBe(false);
...
Redshift and ClickHouse for analytics. Performance and memory optimization. Microservices architecture.
    ...
    it("does not set Go/data-infra gap when profile shows Go and streaming production evidence", () => {
        const profile = {
            ...
            flagshipProjects: userProfile.flagshipProjects.map((p) => p.name === "Backend-leaning full-stack product features"
        ...
        const rules = evaluateRules(makeJob({
            ...
            rawText: "Primary backend in Go. Kafka streaming pipelines. Snowflake warehouse. Distributed microservices.",
        }), profile);
        expect(rules.goDistributedDataInfraRole).toBe(true);
