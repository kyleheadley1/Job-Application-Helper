...
describe("triage from raw pasted text (no structured hand-build)", () => {
    ...
    it("B. strong product startup: higher stack fit than bank, salary parsed, SWE resume", async () => {
        ...
        expect(Object.keys(good.rules.penaltyVector ?? {}).length).toBeLessThan(Object.keys(bad.rules.penaltyVector ?? {}).length);
    ...
    it("C. forward-deployed / integration role: SIE resume", async () => {
    ...
    it("D. explicit early-career role: EARLY_CAREER resume", async () => {
