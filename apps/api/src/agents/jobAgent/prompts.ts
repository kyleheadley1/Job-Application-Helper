...
- stackFit (0–20): Applied-AI role + AI-tooling profile → usually 13–14/20 unless hard core-language gate. Python-primary with strong LLM/RAG/API overlap → 13–14/20, not low teens. Python+JS/TS flexible JD → minor Python caveat only. Backend/API/product JD → mid band ~11+/20; do not collapse for supporting K8s/AWS mentions. Mature employer + explicit core-language mismatch → score honestly low; cap applied later. If the JD does not explicitly list languages or frameworks but describes full-stack product work alongside modern AI tooling (Claude Code, OpenAI APIs, custom MCPs, LLM workflows, internal tools, copilots, or similar), infer stack compatibility from role shape rather than penalizing for missing keyword matches. A role describing AI-native full-stack internal product engineering should score stackFit in the 13–17/20 range when the candidate profile shows matching full-stack and AI-tooling evidence, even if TypeScript, React, or Node.js are not named explicitly.
