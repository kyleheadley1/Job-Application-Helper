...
export const evaluateRules = (job: ExtractedJobData, _profile: UserProfile): RuleEvaluation => {
  const notes: string[] = [];
  ...
  const strictNewGradPipeline =
    hasStrongPipelineMarkers(combinedText) && (harshEmployerContext || explicitDegreeRisk);
  ...
  const seniorityOverreach =
    includesAny(combinedText, ["senior", "staff", "principal"]) || (job.yearsExperience?.min ?? 0) >= 4;

  const locationMismatch =
    (job.remoteType === "onsite" || job.remoteType === "hybrid") && job.locationIsCommutable === false;
  ...
  const citizenshipMismatch =
    Boolean(job.citizenshipRequirement) || RAW_CITIZENSHIP.test(job.rawText ?? "");

  const clearanceMismatch = Boolean(job.clearanceRequirement) || RAW_CLEARANCE.test(job.rawText ?? "");

  const stackMismatch =
    !includesAny(combinedText, ["typescript", "javascript", "node", "react", "api", "full-stack"]) &&
  ...
  if (strictNewGradPipeline) {
    ...
    penaltyVector.earlyCareerSoft = scoringPolicy.hardPenalties.earlyCareerSoftMismatch;
...
export const isLikelyTraditionalEmployer = (job: ExtractedJobData): boolean =>
  includesAny(normalizeText(`${job.company} ${job.title} ${(job.domainTags ?? []).join(" ")} ${job.rawText ?? ""}`), [
    "bank",
    "financial",
    "insurance",
    "institution",
